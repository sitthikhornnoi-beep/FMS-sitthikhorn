...
export async function exportUsersCsvAction(input?: unknown): Promise<ActionResult<{ csv: string; filename: string; count: number }>> {
  return runAction(async () => {
    const ctx = await requirePermission(P.usersRead);
    const parsed = exportUsersQuerySchema.parse(input ?? {}, await em());
    return svc.exportUsersToCsv(ctx.tenantId, parsed);
  });
}
...
export async function importUsersCsvAction(input: unknown): Promise<ActionResult<svc.ImportUsersResult>> {
  return runAction(async () => {
    const ctx = await requirePermission(P.usersManage);
    const parsed = importUsersSchema.parse(input, await em());
    return svc.importUsersFromCsv(actorOf(ctx), parsed);
  });
}
